...
export function setAllLoopKeys(rgb:WootingRgb, colorName) {
    ...
    for(const key of keyMap){
        if(key.x_control && key.x_control['none'] && key.x_control['none'].includes('select_loop')){
...
export function setKeyParamRgb(rgb:WootingRgb, paramName:string, colorName:string) {
    const colorArray = rgb.colorArray;

    const key = keyMap.find( k => k.param_name === paramName);
    if(key) colorArray[key.row][key.column] = (rgbMap[colorName]);
}